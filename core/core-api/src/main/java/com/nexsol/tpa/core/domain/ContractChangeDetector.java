    ...
    public List<ChangeDetail> detect(TotalFormMemberEntity entity, InsuredInfo info, InsuredContractInfo contract) {
        ...
        if (contract != null) {
            ...
            if (entity.getCoverage() != null) {
                ...
                compare("재고자산 가입 금액",entity.getCoverage().getInsuranceCostInven(),contract.insuranceCostInven(),changes);
                compare("야외간판 가입 금액",entity.getCoverage().getInsuranceCostShopSign(),contract.insuranceCostShopSign(),changes);
                compare("자기부담금 가입 금액",entity.getCoverage().getInsuranceCostDeductible(),contract.insuranceCostDeductible(),changes);
            }


            if(entity.getPremium()!=null){
                compare("총보험료",entity.getPremium().getTotalInsuranceCost(),contract.totalInsuranceCost(),changes);
                compare("본인부담 보험료",entity.getPremium().getTotalInsuranceMyCost(),contract.totalInsuranceMyCost(),changes);
                compare("정부지원 보험료",entity.getPremium().getTotalGovernmentCost(),contract.totalGovernmentCost(),changes);
                compare("지자체지원 보험료",entity.getPremium().getTotalLocalGovernmentCost(),contract.totalLocalGovernmentCost(),changes);
            }
        }
