...
public class UnifiedFreeContractParser implements FreeContractParser {
    ...
    public boolean supports(Set<String> headers) {
        return headers.contains("보험사명") && headers.contains("사업자번호")
                && headers.contains("상호명") && headers.contains("증권번호")
                && headers.contains("보험시작일") && headers.contains("보험종기일");
    }
    ...
    public List<FreeContractUpdateInfo> parse(Sheet sheet, Map<String, Integer> headerMap) {
        ...
        for (int i = 1; i <= sheet.getLastRowNum(); i++) {
            ...
            String address = headerMap.containsKey("주소")
                    ? cellTool.getValueAsString(row, headerMap.get("주소"))
                    : "";
            ...
            String errorReason = headerMap.containsKey("오류사유")
                    ? cellTool.getValueAsString(row, headerMap.get("오류사유"))
                    : "";
