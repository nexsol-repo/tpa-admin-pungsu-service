...
각 계약에는 `displayStatus`(신청완료/가입완료/만기임박/기간만료/임의해지/가입오류)가 포함됩니다.
...
=== 계약 상세보기
특정 계약의 상세 정보(가입자 정보, 보험 계약 상세, 결제/환불 정보)를 조회합니다.
...
=== 재가입 이력 조회
특정 계약의 재가입 이력을 조회합니다. 최초가입(0회차)부터 N차 재가입까지의 전체 이력을 역순(최신순)으로 반환합니다.
현재 조회 중인 계약은 `isCurrent=true`로 표시되며, 이전 계약은 `contractId`를 통해 해당 계약 상세로 이동할 수 있습니다.

==== Curl Request
include::{snippets}/admin-renewal-history/curl-request.adoc[]

==== HTTP Request
include::{snippets}/admin-renewal-history/http-request.adoc[]

==== Path Parameters
include::{snippets}/admin-renewal-history/path-parameters.adoc[]

==== HTTP Response
include::{snippets}/admin-renewal-history/http-response.adoc[]

==== Response Fields
include::{snippets}/admin-renewal-history/response-fields.adoc[]

=== 풍수해 알림(문자+메일) 발송
Memo API를 통해 문자(SMS)와 메일을 발송하고 이력을 저장합니다.
==== Curl Request
...
=== 대량 발송 대상 조회 (Preview)
보험종료일/시작일/신청일 기준으로 대량 재가입 안내 발송 대상(기간만료/만기임박) 건수를 조회합니다.
발송 전 대상 건수 확인 용도입니다.

==== Curl Request
include::{snippets}/admin-bulk-notification-preview/curl-request.adoc[]

==== HTTP Request
include::{snippets}/admin-bulk-notification-preview/http-request.adoc[]

==== Query Parameters
include::{snippets}/admin-bulk-notification-preview/query-parameters.adoc[]

==== HTTP Response
include::{snippets}/admin-bulk-notification-preview/http-response.adoc[]

==== Response Fields
include::{snippets}/admin-bulk-notification-preview/response-fields.adoc[]

=== 대량 재가입 안내 발송
Preview에서 확인한 대상에게 문자+메일을 일괄 발송합니다.
50건 단위 배치 처리되며, displayStatus에 따라 만기임박(REJOIN)/기간만료(EXPIRED) 템플릿이 자동 적용됩니다.

==== Curl Request
include::{snippets}/admin-bulk-notification-send/curl-request.adoc[]

==== HTTP Request
include::{snippets}/admin-bulk-notification-send/http-request.adoc[]

==== Request Fields
include::{snippets}/admin-bulk-notification-send/request-fields.adoc[]

==== HTTP Response
include::{snippets}/admin-bulk-notification-send/http-response.adoc[]

==== Response Fields
include::{snippets}/admin-bulk-notification-send/response-fields.adoc[]
...
=== 무료계약 통합 엑셀 업로드
보험사로부터 수령한 무료계약 확정 리스트를 통합 템플릿(엑셀)으로 업로드하여 가입 대기 계약들을 일괄 처리합니다.

**통합 템플릿 헤더**: 보험사명, 사업자번호, 상호명, 주소, 증권번호, 보험시작일, 보험종기일, 총보험료, 국가부담, 지자체부담, 개인부담, 오류사유

**처리 규칙**:

* `오류사유` 컬럼이 비어있으면 → 가입완료(`joinCheck=Y`) 처리
* `오류사유` 컬럼에 내용이 있으면 → 가입오류(`joinCheck=F`) 처리
* 대상: `joinCheck=N`(보험접수완료) AND `payYn=N`(무료) 상태인 계약만 매칭
